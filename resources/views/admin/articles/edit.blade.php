...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-600">Perbarui konten artikel dan pengaturan publikasi</p>
        </div>
        ...
        <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Opsional. Jika dikosongkan, otomatis menggunakan "Redaksi HalimiNews".</p>
                    @error('author_name')
            ...
            <div>
                ...
                <textarea name="excerpt" id="excerpt" rows="3" required
                          class="w-full px-3 py-2 border @error('excerpt') border-red-500 @else border-gray-300 @enderror rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">{{ old('excerpt', $article->excerpt) }}</textarea>
                @error('excerpt')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="content" id="content" rows="10" required
                          class="w-full px-3 py-2 border @error('content') border-red-500 @else border-gray-300 @enderror rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm">{{ old('content', $article->content) }}</textarea>
                @error('content')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-100">
                <!-- Thumbnail -->
                <div>
                    <label for="thumbnail" class="block text-sm font-medium text-gray-700 mb-1">Ubah Thumbnail (Opsional)</label>
                    @if ($article->thumbnail)
                        <div class="mb-3 flex items-center space-x-3">
                            <img src="{{ asset('storage/' . $article->thumbnail) }}" alt="{{ $article->title }}" 
                                 class="w-20 h-14 object-cover rounded border border-gray-200">
